...
用法:
    python finetune/eval/baseline.py
    python finetune/eval/baseline.py --model qwen3:latest --prompts finetune/eval/prompts_eval.txt

注意：base 用运行时 system prompt（configs/persona.yaml），以还原"未微调、只靠
prompt 撑人格"的真实基线——这正是要对照的对象。
"""
...
def main() -> None:
    ...
    ap.add_argument("--with-persona-prompt", action="store_true", default=True,
                    help="是否注入 persona system prompt（默认 True，还原真实基线）")
    args = ap.parse_args()
    ...
    system = None
    if args.with_persona_prompt:
        persona = Persona(cfg.persona.get("card", {"name": "oki"}),
                          PersonaKnobs(**cfg.persona.get("knobs", {})))
        system = persona.system_prompt()

    out = args.out or Path("finetune/output") / f"baseline_{args.model.replace(':', '_').replace('/', '_')}.jsonl"
    out.parent.mkdir(parents=True, exist_ok=True)

    stamp = datetime.now().isoformat(timespec="seconds")
    print(f"基线采集：model={args.model}  prompts={len(prompts)}  -> {out}\n")
    with out.open("w", encoding="utf-8") as f:
        for i, p in enumerate(prompts, 1):
            ...
            dt = time.time() - t0
            rec = {"model": args.model, "timestamp": stamp, "prompt": p,
